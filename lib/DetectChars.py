...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaClassifications = np.loadtxt("/srv/www/pyms/config/classifications.txt", np.float32)                  # read in training classifications
    except:                                                                                 # if file could not be opened
        print 0                # show error message
    
        return False                                                                        # and return False
    # end try

    try:
        npaFlattenedImages = np.loadtxt("/srv/www/pyms/config/flattened_images.txt", np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
        print 0               # show error message
